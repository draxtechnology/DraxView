...
```
dotnet run --project DraxView/DraxView.csproj
```

Then open http://localhost:5080. First run creates `users.json` with the account `admin` / `admin`; change it from the Settings page.

Broker address, port and topic prefix live in `DraxView/appsettings.json`.

## How it fits the service

| Direction | Topic | Payload |
|-----------|-------|---------|
| Service to DraxView | `drax/<panel>/event` | JSON from MqttTransfer.PublishEvent (type, on, decoded node/loop/input, text) |
| Service to DraxView | `drax/<panel>/log` | Human-readable log line |
| DraxView to service | `drax/<panel>/cmd` | The same pipe-command strings the WinForms client sends, e.g. `SILENCE\|0,0,0,0` |
